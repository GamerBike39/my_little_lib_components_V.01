...
type ContentListMultipleImageProps = {
  title?: string;
  subtitle?: string;
...
export const ContentListMultipleImage = ({
  title,
  subtitle,
  introduceTitle,
  content,
  pictures,
}: ContentListMultipleImageProps) => {
  return (
        ...
        <div className="grid grid-cols-2 gap-5">
          {pictures?.map((item, index) => (
            <img
              key={index}
              className={cn(
                "object-cover w-full rounded shadow-lg",
                index === 0 ? "col-span-2 h-56" : "h-48"
              )}
              src={item.img}
              alt={item.alt}
            />
